...
public class CapitalAction extends BaseAction {
	...
	public void loadSave() throws IOException {
		...
		PageView<Pro_Capital_In> pageView = new PageView<Pro_Capital_In>(rows,
				page);
		...
		QueryResult<Pro_Capital_In> queryResult = capitalService.getScrollData(
				firstindex, rows, where.toString(), parameters.toArray());
		long total = capitalService.getScrollData(-1, -1, where.toString(),
	...
	public void loadSubmit() throws IOException {
		...
		List<Object> parameters = new ArrayList<Object>();
		where
				.append(" ((((o.project.iVerify=?4 or o.project.iVerify=?1 ) or o.project.iVerify=?5)and o.project.cState=?2)or o.project.cState>?2 )and o.project.projectGroup=?3 ");
		...
		for (Pro_Capital_In p : list) {
			...
			if (project.getcState() > Status.CAPI)
				state = "通过";
			jo1.put("state", state);
	...
	public void deleteRow() throws IOException {
		...
		Map map = new HashMap();
		if (id != null) {
			ProjectInfo project = capitalService.find(id).getProject();
			projectInfoService.editIverfy(project.getId(), Iverfy.SAVE);
			capitalService.delete(Integer.valueOf(id));
	...
	public void exsitProject() throws IOException {
		...
		if (loginUser.getProjectGroup() == null) {
			...
			List<ProjectInfo> projectInfo = projectInfoService.findProject(
					group.getId(), Status.CAPI, Iverfy.SAVE);
	...
	public void add() throws IOException {
		...
		String capplicant = ServletActionContext.getRequest().getParameter(
				"capplicant");
		Integer dscale = Integer.valueOf(ServletActionContext.getRequest()
				.getParameter("dscale"));
		String creason = ServletActionContext.getRequest().getParameter(
				"creason");
		String cdemand = ServletActionContext.getRequest().getParameter(
				"cdemand");
		String id = ServletActionContext.getRequest().getParameter("id");
		Pro_Capital_In entity = new Pro_Capital_In();
		entity.setCapplicant(capplicant);
		entity.setDscale(dscale);
		entity.setCreason(creason);
		entity.setCdemand(cdemand);
	...
	public void exportExcel(String fileName, List<Pro_Capital_In> content) {
		...
		try {
			...
			ws.addCell(new Label(2, 0, "申请人", wcf));
			ws.addCell(new Label(3, 0, "规模", wcf));
			ws.addCell(new Label(4, 0, "原因", wcf));
			ws.addCell(new Label(5, 0, "需求", wcf));
			...
			for (int i = 0; i < content.size(); i++) {
				...
				ws.addCell(new Label(2, i + 1, content.get(i).getCapplicant(),
						contentWCF));
				ws.addCell(new Label(3, i + 1, content.get(i).getDscale()
						.toString(), contentWCF));
				ws.addCell(new Label(4, i + 1, content.get(i).getCreason(),
						contentWCF));
				ws.addCell(new Label(5, i + 1, content.get(i).getCdemand(),
						contentWCF));
	...
	public void excelSave() {
		...
		parameters.add(group);
		List<Pro_Capital_In> content = capitalService.getScrollData(-1, -1,
