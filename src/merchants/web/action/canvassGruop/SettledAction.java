...
@Controller
@Scope("prototype")
public class SettledAction extends BaseAction {
	...
	public void loadSave() throws IOException {
		...
		PageView<Pro_Settled> pageView = new PageView<Pro_Settled>(rows, page);
		...
		QueryResult<Pro_Settled> queryResult = settledService.getScrollData(
				firstindex, rows, where.toString(), parameters.toArray());
		long total = settledService.getScrollData(-1, -1, where.toString(),
	...
	public void loadSubmit() throws IOException {
		...
		for (Pro_Settled p : list) {
			...
			if (project.getcState() > Status.SETT)
				state = "通过";
			jo1.put("state", state);
	...
	public void deleteRow() throws IOException {
		...
		Map map = new HashMap();
		if (id != null) {
			ProjectInfo project = settledService.find(id).getProject();
			projectInfoService.editIverfy(project.getId(), Iverfy.SAVE);
			settledService.delete(Integer.valueOf(id));
	...
	public void exsitProject() throws IOException {
		...
		if (loginUser.getProjectGroup() == null) {
			...
			List<ProjectInfo> projectInfo = projectInfoService.findProject(
					group.getId(), Status.SETT, Iverfy.SAVE);
			for (ProjectInfo p : projectInfo) {
				if (p.getSettled() == null) {
					Map m = new HashMap();
	...
	public void add() throws IOException {
		...
		String cfruit = ServletActionContext.getRequest()
				.getParameter("cfruit");
		String chandlePerson = ServletActionContext.getRequest().getParameter(
				"chandlePerson");
		String chandleSubstance = ServletActionContext.getRequest()
				.getParameter("chandleSubstance");
		String id = ServletActionContext.getRequest().getParameter("id");
		Pro_Settled entity = new Pro_Settled();
		entity.setCfruit(cfruit);
		entity.setChandlePerson(chandlePerson);
		entity.setChandleSubstance(chandleSubstance);
	...
	public void exportExcel(String fileName, List<Pro_Settled> content) {
		WritableWorkbook wwb;
		FileOutputStream fos;
		try {
			fos = new FileOutputStream(fileName);
			wwb = Workbook.createWorkbook(fos);
			WritableSheet ws = wwb.createSheet("落户信息列表", 10); // 创建一个工作表
			// 设置表头单元格的文字格式
			...
			ws.addCell(new Label(2, 0, "办理人员", wcf));
			ws.addCell(new Label(3, 0, "办理事项", wcf));
			ws.addCell(new Label(4, 0, "结果", wcf));
			...
			for (int i = 0; i < content.size(); i++) {
				...
				ws.addCell(new Label(2, i + 1, String.valueOf(content.get(i)
						.getChandlePerson()), contentWCF));
				ws.addCell(new Label(3, i + 1, content.get(i)
						.getChandleSubstance(), contentWCF));
				ws.addCell(new Label(4, i + 1, content.get(i).getCfruit(),
						contentWCF));
	...
	public void excelSave() {
		...
		parameters.add(group);
		List<Pro_Settled> content = settledService.getScrollData(-1, -1,
