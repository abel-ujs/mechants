...
import merchants.utils.grcommon;
import merchants.web.action.base.BaseAction;

public class GenXmlData {

	private static String driver;// mssql2005
	...
	static {
		try {
			properties.load(BaseAction.class.getClassLoader()
	...
	public static void GenNodeXmlData(HttpServletResponse response,
		...
		try {
			try {
				...
				java.sql.Statement stmt = con.createStatement(); // 用于发送简单的SQL语句
				...
				if (ToCompress) {
					...
					response.addHeader("gr_zip_encode",
							response.getCharacterEncoding()); // 指定数据的编码方式 utf-8
																// utf-16 ...
	...
	public static String GenReportParameterData(String ParameterQuerySQL) throws Exception {
		StringBuffer XmlText = new StringBuffer("<_grparam>\n");

		Class.forName(driver); // Class.forName 装载驱动程序
		Connection con = DriverManager.getConnection(url, user, password); // 用适当的驱动程序类与
																			// DBMS
																			// 建立一个连接
	...
	public static void BatchGenXmlData(HttpServletResponse response,
			HttpSession session, String SessionItemName, String QuerySQL,
			int StartNo, int WantRows, boolean ToCompress) {
		try {
			try {
				response.resetBuffer();
				...
				ResultSet rs = (ResultSet) session
						.getAttribute(SessionItemName);
				if (rs == null) {
					Class.forName(driver); // Class.forName 装载驱动程序
					Connection con = DriverManager.getConnection(url, user,
							password); // 用适当的驱动程序类与 DBMS 建立一个连接
