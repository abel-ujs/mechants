...
<html>
	...
	function reload(){
		$('#dg').datagrid('load',{url:'/group/se/indexSubmit'});
	}
	...
	function initdatagrid(){
		$("#dg").datagrid({
			...
			columns:[[
				...
				{field:'chandlePerson',title:'办理人员',width:100},
				{field:'chandleSubstance',title:'办理事项',width:200},
				{field:'cfruit',title:'结果',width:200},
	...
	$.post('/group/se/excelSubmit', {fileName:'settled.xls'}, function(data) {
	...
	function edit(){
		...
		if (row) {
		$.post('/group/se/isEdit',{'cProCode':row.cProCode},function(data,status){
			var result=$.parseJSON(data);
			if(result.errorMsg == ''){
				$('#dlg').dialog('open').dialog('setTitle', '编辑研判信息');
				$('#cc').combobox({  
					url:'/group/se/loadEditProject?cProCode='+row.cProCode,
   				 	valueField:'id',  
  				  	textField:'cProName',
				...
				$('#fm').form('load', row);
				url = '/group/se/add?id=' + row.id+'&type=2';
	...
	function add(){
		$.post('/group/se/exsitProject','',function(data,status){
		var result=$.parseJSON(data);
		if(result.errorMsg=='') {
		...
		<div id="dlg" buttons="#dlg-buttons">
			<form id="fm" method="post">
				...
				<div align="center">
					...
					<input name="chandlePerson"/>
				</div>
				<div align="center">
					<label>
						办理事项
					</label>
					<textarea rows="5" cols="50" name="chandleSubstance"></textarea>
				</div>
				<div align="center">
					<label>
						办理结果
					</label>
					<textarea rows="5" cols="50" name="cfruit"></textarea>
				</div>
				<div align="center">
					<div id="filename">	
					</div>
