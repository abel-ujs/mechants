...
<html>
	...
	function reload(){
		$('#dg').datagrid('load',{url:'/group/ne/indexSave'});
	}
	...
	function initdatagrid(){
		$("#dg").datagrid({
			...
			columns:[[
				...
				{field:'cdate',title:'日期',width:100,sortable:'true',formatter: function(value,row,index){
	...
	$.post('/group/ne/excelSave', {fileName:'negotiate.xls'}, function(data) {
	...
	function edit(){
		...
		if (row) {
			...
			$('#cc').combobox({  
					url:'/group/ne/loadEditProject?cProCode='+row.cProCode,
   				 	valueField:'id',  
  				  	textField:'cProName'  
			});
			$('#cc').attr('readonly','readonly');
			$('#fm').form('load', row);
			url = '/group/ne/add?id=' + row.id+'&type=2';
		}else{
	...
	function add(){
		$.post('/group/ne/exsitProject','',function(data,status){
		var result=$.parseJSON(data);
		if(result.errorMsg=='') {
			$('#dlg').dialog('open').dialog('setTitle', '新建表单');
			$('#cc').combobox({data:result.project}); 
			$('#fm').form('clear');
			url = '/group/ne/add?type=1';
			}
